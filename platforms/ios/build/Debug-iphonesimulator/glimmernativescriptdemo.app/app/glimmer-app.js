    ...
    */
    ...
    class RuntimeCompilerLoader {
        ...
        async getTemplateIterator(app, env, builder, dynamicScope, self) {
            ...
            return Promise.resolve(render(program, env, self, dynamicScope, builder, mainLayout.asLayout().compile()));
        }
    }
    ...
    class GlimmerNativescriptDemo extends Component {}

    var __ui_components_GlimmerNativescriptDemo_template__ = { "id": "Qz+Z/Z0R", "block": "{\"symbols\":[],\"statements\":[[5,\"PageOne\",[],[[],[]],[[\"default\"],[{\"statements\":[],\"parameters\":[]}]]]],\"hasEval\":false}", "meta": { "specifier": "template:/glimmer-nativescript-demo/components/GlimmerNativescriptDemo" } };

    const CONSTANT$1 = 0;
    const INITIAL$1 = 1;
    const VOLATILE$1 = NaN;
    class RevisionTag$1 {
        validate(snapshot) {
            return this.value() === snapshot;
        }
    ...
    class TagWrapper$1 {
        ...
        value() {
            let func = VALUE$1[this.type];
            return func(this.inner);
        }
        validate(snapshot) {
            let func = VALIDATE$1[this.type];
            return func(this.inner, snapshot);
        }
    }
    function register$1(Type) {
        let type = VALUE$1.length;
        VALUE$1.push(tag => tag.value());
        VALIDATE$1.push((tag, snapshot) => tag.validate(snapshot));
        Type.id = type;
    }
    ///
    // CONSTANT: 0
    VALUE$1.push(() => CONSTANT$1);
    VALIDATE$1.push((_tag, snapshot) => snapshot === CONSTANT$1);
    const CONSTANT_TAG$1 = new TagWrapper$1(0, null);
    // VOLATILE: 1
    VALUE$1.push(() => VOLATILE$1);
    VALIDATE$1.push((_tag, snapshot) => snapshot === VOLATILE$1);
    const VOLATILE_TAG$1 = new TagWrapper$1(1, null);
    // CURRENT: 2
    VALUE$1.push(() => $REVISION$1);
    VALIDATE$1.push((_tag, snapshot) => snapshot === $REVISION$1);
    ///
    let $REVISION$1 = INITIAL$1;
    class DirtyableTag$1 extends RevisionTag$1 {
        static create(revision = $REVISION$1) {
            return new TagWrapper$1(this.id, new DirtyableTag$1(revision));
        }
        constructor(revision = $REVISION$1) {
            super();
            this.revision = revision;
        }
        value() {
    ...
    register$1(DirtyableTag$1);
    function combine$1(tags) {
        let optimized = [];
        for (let i = 0, l = tags.length; i < l; i++) {
            let tag = tags[i];
            if (tag === VOLATILE_TAG$1) return VOLATILE_TAG$1;
            if (tag === CONSTANT_TAG$1) continue;
            optimized.push(tag);
        }
        return _combine$1(optimized);
    }
    function _combine$1(tags) {
        switch (tags.length) {
            case 0:
                return CONSTANT_TAG$1;
            case 1:
                return tags[0];
            case 2:
                return TagsPair$1.create(tags[0], tags[1]);
            default:
                return TagsCombinator$1.create(tags);
        }
    }
    class CachedTag$1 extends RevisionTag$1 {
        constructor() {
            super(...arguments);
            this.lastChecked = null;
            this.lastValue = null;
        ...
        value() {
            ...
            if (lastChecked !== $REVISION$1) {
                this.lastChecked = $REVISION$1;
    ...
    class TagsPair$1 extends CachedTag$1 {
        static create(first, second) {
            return new TagWrapper$1(this.id, new TagsPair$1(first, second));
        }
        constructor(first, second) {
            super();
            this.first = first;
    ...
    register$1(TagsPair$1);
    class TagsCombinator$1 extends CachedTag$1 {
        static create(tags) {
            return new TagWrapper$1(this.id, new TagsCombinator$1(tags));
        }
        constructor(tags) {
            super();
            this.tags = tags;
    ...
    register$1(TagsCombinator$1);
    class UpdatableTag$1 extends CachedTag$1 {
        static create(tag) {
            return new TagWrapper$1(this.id, new UpdatableTag$1(tag));
        }
        constructor(tag) {
            super();
            this.tag = tag;
            this.lastUpdated = INITIAL$1;
        }
        compute() {
            return Math.max(this.lastUpdated, this.tag.value());
        }
        update(tag) {
            if (tag !== this.tag) {
                this.tag = tag;
                this.lastUpdated = $REVISION$1;
                this.invalidate();
            }
        }
    }
    register$1(UpdatableTag$1);
    class UpdatableDirtyableTag extends CachedTag$1 {
        static create(tag = CONSTANT_TAG$1) {
            return new TagWrapper$1(this.id, new UpdatableDirtyableTag(tag));
        }
        constructor(tag, revision = $REVISION$1) {
            super();
            this.tag = tag;
            this.lastUpdated = INITIAL$1;
            this.revision = revision;
    ...
    register$1(UpdatableDirtyableTag);
    ...
    const EPOCH$1 = DirtyableTag$1.create();
    ...
    class Tracker$2 {
        ...
        combine() {
            ...
            if (tags.size === 0) return CONSTANT_TAG$1;
            return combine$1([...tags]);
        }
    }
    function tracked$1(...args) {
        let target = args[0],
            key = args[1],
            descriptor = args[2];
        ...
        if (descriptor) {
            return descriptorForTrackedComputedProperty$1(target, key, descriptor);
        } else {
            installTrackedProperty$1(target, key);
        }
    }
    /**
     * Whenever a tracked computed property is entered, the current tracker is
     ...
     * itself, including child tracked computed properties.
     */
    let CURRENT_TRACKER$2 = null;
    function descriptorForTrackedComputedProperty$1(target, key, descriptor) {
        ...
        function getter() {
            ...
            let old = CURRENT_TRACKER$2;
            let tracker = CURRENT_TRACKER$2 = new Tracker$2();
            // Call the getter
            let ret = get.call(this);
            // Swap back the parent tracker
            CURRENT_TRACKER$2 = old;
            // Combine the tags in the new tracker
            let tag = tracker.combine();
            if (CURRENT_TRACKER$2) CURRENT_TRACKER$2.add(tag);
            // Update the UpdatableTag for this property with the tag for all of the
            // consumed dependencies.
            metaFor$1(this).updatableTagFor(key).inner.update(tag);
            return ret;
        }
        function setter() {
            EPOCH$2.inner.dirty();
            // Mark the UpdatableTag for this property with the current tag.
            metaFor$1(this).updatableTagFor(key).inner.update(DirtyableTag$1.create());
    ...
    function trackedGet$1(obj, key) {
        if (CURRENT_TRACKER$2) CURRENT_TRACKER$2.add(metaFor$1(obj).updatableTagFor(key));
    }
    /**
      ...
      from it.
     */
    function installTrackedProperty$1(target, key) {
    ...
    class Meta$1 {
        constructor(parent) {
            ...
            this.trackedComputedProperties = parent ? Object.create(parent.trackedComputedProperties) : dict$1();
        }
        /**
         * The tag representing whether the given property should be recomputed. Used
         * by e.g. Glimmer VM to detect when a property should be re-rendered. Think
        ...
        tagFor(key) {
            ...
            return this.tags[key] = DirtyableTag$1.create();
        }
        /**
         * The tag used internally to invalidate when a tracked property is set. For
         * static properties, this is the same UpdatableTag returned from `tagFor`.
        ...
        updatableTagFor(key) {
            ...
            if (isComputed) {
                ...
                return this.computedPropertyTags[key] = UpdatableTag$1.create(CONSTANT_TAG$1);
            } else {
                // The key is for a static property.
                tag = this.tags[key];
                if (tag) {
                ...
                return this.tags[key] = UpdatableTag$1.create(CONSTANT_TAG$1);
            }
        }
    }
    /**
    ...
    function metaFor$1(obj) {
        ...
        // parent Meta to inherit from. If we reach the end of the chain and have not
        // found a Meta, there is nothing to inherit.
        let protoMeta = findPrototypeMeta$1(obj);
        meta = new Meta$1(protoMeta);
        // Save the object's Meta and return it.
        META_MAP$1.set(obj, meta);
        return meta;
    }
    const getPrototypeOf$1 = Object.getPrototypeOf;
    // Finds the nearest Meta instance in an object's prototype chain. Returns null
    // if the end of the prototype chain is reached without finding a Meta.
    function findPrototypeMeta$1(obj) {
        let meta = null;
        let proto = obj;
        while (!meta) {
            ...
            meta = META_MAP$1.get(proto);
        }
        return meta;
    }
    const EPOCH$2 = DirtyableTag$1.create();
    let propertyDidChange$1 = function propertyDidChange() {};
    ...
    __decorate([tracked$1], PageOne.prototype, "title", void 0);

    var __ui_components_PageOne_template__ = { "id": "/cEzipdT", "block": "{\"symbols\":[],\"statements\":[[7,\"page\"],[9],[0,\"\\n    \"],[7,\"actionbar\"],[9],[0,\"\\n        \"],[7,\"label\"],[12,\"text\",[24,0,[\"title\"]],null],[11,\"class\",\"action-label\",null],[9],[10],[0,\"\\n    \"],[10],[0,\"\\n    \"],[7,\"button\"],[12,\"text\",[24,0,[\"buttonText\"]],null],[11,\"class\",\"btn\",null],[9],[3,\"on\",[\"tap\",[29,\"action\",[[25,[\"buttonClick\"]]],null]],null],[10],[0,\"\\n\"],[10]],\"hasEval\":false}", "meta": { "specifier": "template:/glimmer-nativescript-demo/components/PageOne" } };

    var moduleMap = { 'component:/glimmer-nativescript-demo/components/GlimmerNativescriptDemo': GlimmerNativescriptDemo, 'template:/glimmer-nativescript-demo/components/GlimmerNativescriptDemo': __ui_components_GlimmerNativescriptDemo_template__, 'component:/glimmer-nativescript-demo/components/PageOne': PageOne, 'template:/glimmer-nativescript-demo/components/PageOne': __ui_components_PageOne_template__ };

    var resolverConfiguration = { "app": { "name": "glimmer-nativescript-demo", "rootName": "glimmer-nativescript-demo" }, "types": { "application": { "definitiveCollection": "main" }, "component": { "definitiveCollection": "components" }, "component-test": { "unresolvable": true }, "helper": { "definitiveCollection": "components" }, "helper-test": { "unresolvable": true }, "renderer": { "definitiveCollection": "main" }, "template": { "definitiveCollection": "components" } }, "collections": { "main": { "types": ["application", "renderer"] }, "components": { "group": "ui", "types": ["component", "component-test", "template", "helper", "helper-test"], "defaultType": "component", "privateCollections": ["utils"] }, "styles": { "group": "ui", "unresolvable": true }, "utils": { "unresolvable": true } } };
    ...
    app.renderComponent('GlimmerNativescriptDemo', containerElement, null);
    app.boot();
